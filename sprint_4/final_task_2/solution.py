...
# > Нам известны число элементов в корзине M, а также максимальное количество ключей в таблице N.
# Так как в качестве метода разрешения коллизий был выбран метод цепочек, то:
# (T - длина цепочки, ее средня длина равна примерно N/M)
# - Время поиска элемента O(1) для get - в лучшем случае, O(T) - в худшем случае(линейный поиск)
# - Время вставки элемента O(1) для put, так как вставка происходит в начало связного списка
# - Время поиска элемента: O(1) для remove - в лучшем случае, O(T) - в худшем случае(линейный поиск)
# Учитывая выбранную хэш-функцию можно сказать, что индекс функция также будет равноменрной.
# Тогда можно сказать что средняя сложность операций put, remove, get в хеш-таблице будет:
# O(1+N/M) = O(1+a), где a - коэффициент заполнения таблицы (справедливо для метода цепочек).
# Основной вывод: играясь размером корзин мы можем регулировать средним временем
# выполнения операций (при фиксированном N).
# Получается tradeoff: при большом M операции будут выполняться быстрее и коллизий будут меньше,
# так как мы уменьшаем среднюю длину цепочек, однако мы будем потреблять больше памяти.
#
#
# 4. Пространственная сложность
# > Определяется размером таблицы M и затратами на хранение элементов в связных списках цепочек.
# Также мы тратим O(n) памяти на хранение комманд введенным пользователем.
# - Затраты на хранение хеш-таблицы: O(M)
# - Затраты на хранение цепочек: O(N), где N - максимальное количество ключей в таблице:
# Если все ключи попадут в одну корзину, имеем один связный список длиной N. Если все ключи разные,
# то имеем N связных списков с одним узлом. В любом случае потребление памяти здесь будет O(N).
#
# Итого, имеем: O(M + N + n)
#
#
