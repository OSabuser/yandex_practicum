...
Предположим, дан массив чисел, который нужно отсортировать с помощью алгоритма быстрой сортировки. Но вдруг оказалось, что массив уже отсортирован.

![](pics/p21.png)

Допустим, вы берёте самый первый элемент массива в качестве опорного.

![](pics/p22.png)

Итак, на первом шаге опорный элемент $0$. Чтобы получить три подмассива, нужно сравнить с опорным $(n−1)$ элемент.

![](pics/p23.png)

На втором шаге примем за опорный элемент $3$. Будет выполнено $(n−2)$ операции сравнения. На каждом шаге количество сравнений будет уменьшаться на 1. Когда останется 1 элемент в массиве, сравнивать его будет не с чем. Поэтому получится 0 операций сравнения.
...
Если подставить в формулу получившиеся для алгоритма значения $a_{1}=n−1;  a_{n}=0;$

$\frac {n^{2}-n}{2}=O(n^{2})$

#### 3.8.3 Сортировка отсортированного массива

Представим эту же задачу другим, более наглядным способом.

Мы можем изобразить это в виде схемы рекурсивной обработки. На каждом шаге элемент сравнивается с опорным и перемещается в левую или правую половину. Это занимает $O(1)$ операций. Поэтому можно сказать, что количество операций в процессе сортировки пропорционально количеству элементов в получившемся дереве обработки элементов.

![](pics/p24.png)

На картинке ячейки располагаются на $(n−1)$ уровнях, на каждом из которых в среднем $\frac {n}{2}$ ячеек. А значит, снова получаем, что общее число операций: $O((n-1) \cdot \frac {n}{2} =O(n^{2})$

**Мы получили квадратичную сортировку, которая асимптотически работает не быстрее сортировки пузырьком, вставками или выбором!**

Но не всё так плохо! Посчитаем сложность алгоритма на том же примере, если опорный элемент на каждом шаге выбран удачно и разделит массив пополам.

![](pics/p25.png)

На каждом шаге мы снова проделываем $O(n)$ операций. Количество шагов деления (глубина рекурсии) составляет $O(log⁡n).$ Поэтому общее число операций будет $O(nlog⁡n)$.

#### 3.8.4 Варианты выбора опорного элемента

- Одного примера недостаточно, чтобы понять, почему алгоритм обычно работает быстрее своей худшей оценки. Чтобы посчитать сложность в среднем, нужно учесть все возможные варианты входных данных и усреднить по ним время работы алгоритма.

- Допустим, нужно найти определённый элемент в массиве. Сложность этой операции в худшем случае $O(n)$. Худший случай — когда искомый элемент находится в конце массива или вовсе отсутствует.
...
- Эти случаи маловероятны. Нам интересен ожидаемый результат, когда нужный элемент находится в произвольном месте массива. В среднем понадобится $\frac {n}{2}$  операций. Значит, сложность этого алгоритма в среднем — $O(\frac {n}{2})$, то есть $O(n)$..

- Для некоторых алгоритмов сложность в худшем случае и сложность в среднем не совпадают.

> [!IMPORTANT]
>
> Сложность быстрой сортировки в среднем — такая же, как в лучшем случае: $O(nlog⁡n)$. Это не так-то просто доказать, так что я расскажу только основную идею.

К сожалению, нет быстрого способа всегда выбирать опорный элемент оптимальным образом, поэтому во многих случаях дерево будет иметь глубину больше чем $log⁡_{2}n$. Но можно выбирать опорный элемент так, чтобы он случайным образом разбивал массив на части. Каждая последовательность разбиений будет давать своё дерево обработки. Можно показать, что вероятность получить глубокое дерево очень мала. В среднем высота дерева будет логарифмически зависеть от количества элементов, поэтому общее число операций в среднем получится $O(nlog⁡n)$.p

Посмотрим на примере, как алгоритм работает, если выбирать опорные элементы случайным образом.

![](pics/p26.png)

> Числа 5 и 2 были удачно выбраны в качестве опорных элементов — они разделили массивы на равные части. Число 7 было выбрано менее удачно, оно сократило размер обрабатываемого подмассива не в 2, а лишь в 1.5 раза (с трёх до двух). Из-за этого в правой ветви дерева потребовалось совершить на один рекурсивный вызов больше

- Значит, мы были правы, что выбирали случайный элемент в качестве опорного? 
- Да, это хороший выбор. Другой неплохой стратегией будет взять крайний левый элемент, крайний правый элемент, центральный элемент, а затем взять медиану этих значений (средний элемент). Например, если мы разбиваем на части массив `[3, 7, 9, 14, 1, 2, 5]`, мы возьмём элементы `[3, 14, 5]`. Расположим эти три элемента в порядке возрастания: `[3, 5, 14]`. И возьмём их медиану, то есть число `5`.

- Получается, что хотя сложность в худшем случае $O(n^2)$, на практике алгоритм работает эффективно?
- Да, на практике этот алгоритм работает за$O(nlog⁡n)$.

Но при неудачной реализации выбора опорного элемента, можно подобрать такие входные данные, на которых он будет работать квадратичное время. Они называются киллер-последовательностями (*англ*. killer-sequence). Например, если в качестве опорного выбирается первый или последний элемент, то заранее отсортированный массив будет killer-последовательностью. 

А вот при выборе опорного элемента случайным образом, подобрать гарантированно плохую последовательность не удастся.

### 3.9 Сортировка подсчётом

Сортировка подсчётом (*англ.* counting sort) работает за линейное время. Но дело в том, что у этого алгоритма есть ограничения, из-за которых его использование не всегда возможно.

#### 3.9.1 Принцип работы
...
Закрепим знания псевдокодом. Этот псевдокод будет работать, если значения элементов массива лежат в полуинтервале от 0 до *k*, $[0,k)$:
